...
from app.schemas.token import ResponseToken, RefreshTokenValue
from app.schemas.user import UserCreate, UserLogin, RequestRefreshToken
...
@router.post('/demo', response_model=ResponseToken)
...
def login_for_access_token(json_data: UserLogin, db: Session = Depends(get_db)):
    ...
    logger.info(f"User logged in: {user.username}")
    return {'data': {"accessToken": access_token, "token_type": "bearer", 'username': user.username,
                     'expires': int(time.time() + access_token_expires.seconds), 'refreshToken': access_token}}


@router.post("/refresh-token", response_model=RefreshTokenValue)
def refresh_access_token(token: RequestRefreshToken):
    try:
        ...
        if username is None:
            ...
            raise InvalidTokenException(code=Code.failed, message='Invalid token, username is null')
    except ExpiredSignatureError:
        raise HTTPException(status_code=401, detail='Invalid token, token is expired')
    except JWTError:
        raise HTTPException(status_code=401, detail="Invalid token, decode jwt error")
    ...
    return {'data': {"accessToken": new_token,
                     'expires': int(time.time() + new_token_expires.seconds), 'refreshToken': new_token}}
